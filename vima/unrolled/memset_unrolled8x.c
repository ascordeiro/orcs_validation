    ...
    if (size != 0 && (size & (size - 1)) == 0){
        ...
        if (VECTOR_SIZE == 2048){
            ...
            for (__v32s i = 0; i < v_size; i += VECTOR_SIZE*8) {
                ...
                _vim2K_imovs(1, &vector[i+VECTOR_SIZE*4]);
                _vim2K_imovs(1, &vector[i+VECTOR_SIZE*5]);
                _vim2K_imovs(1, &vector[i+VECTOR_SIZE*6]);
                _vim2K_imovs(1, &vector[i+VECTOR_SIZE*7]);
            }
            ORCS_tracing_stop();
        }
        ...
        if (VECTOR_SIZE == 64){
            ...
            for (__v32s i = 0; i < v_size; i += VECTOR_SIZE*8) {
                ...
                _vim64_imovs(1, &vector[i+VECTOR_SIZE*4]);
                _vim64_imovs(1, &vector[i+VECTOR_SIZE*5]);
                _vim64_imovs(1, &vector[i+VECTOR_SIZE*6]);
                _vim64_imovs(1, &vector[i+VECTOR_SIZE*7]);
            }
            ORCS_tracing_stop();
        }
